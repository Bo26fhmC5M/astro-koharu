...
import type { BeganMotionCallback, FinishedMotionCallback } from '@cubism-framework/motion/acubismmotion';
...
import { CubismLogError, CubismLogInfo } from '@cubism-framework/utils/cubismdebug';
import { Live2DClock } from './clock';
import type { Live2DRenderContext, Live2DTexture } from './contracts';
import { motionGroupIdle, motionPriority } from './motion-constants';
import { readInitialPartOpacities } from './motion-part-opacity';
import { readParameterValues, saveParametersWithTransientState } from './parameter-state';
import { Live2DWavFileHandler } from './wav-file-handler';
...
async function fetchArrayBuffer(filePath: string): Promise<ArrayBuffer> {
...
export class Live2DModel extends CubismUserModel {
  ...
  public async loadAssets(dir: string, fileName: string): Promise<void> {
    ...
    const arrayBuffer = await fetchArrayBuffer(`${this._modelHomeDir}${fileName}`);
  ...
  private loadArrayBuffer(filePath: string, onLoad: (arrayBuffer: ArrayBuffer) => void): void {
    void fetchArrayBuffer(filePath)
      .then(onLoad)
      .catch((error: unknown) => this.failLoad(error));
  }

  private failLoad(error: unknown): void {
    const failure = error instanceof Error ? error : new Error(String(error));
    CubismLogError(failure.message);
    this._rejectLoad?.(failure);
  ...
  private setupModel(setting: ICubismModelSetting): void {
    ...
    if (this._modelSetting.getModelFileName() !== '') {
      ...
      this.loadArrayBuffer(`${this._modelHomeDir}${modelFileName}`, (arrayBuffer) => {
    ...
    const loadCubismExpression = (): void => {
      if (this._modelSetting.getExpressionCount() > 0) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          this.loadArrayBuffer(`${this._modelHomeDir}${expressionFileName}`, (arrayBuffer) => {
    ...
    const loadCubismPhysics = (): void => {
      if (this._modelSetting.getPhysicsFileName() !== '') {
        ...
        this.loadArrayBuffer(`${this._modelHomeDir}${physicsFileName}`, (arrayBuffer) => {
    ...
    const loadCubismPose = (): void => {
      if (this._modelSetting.getPoseFileName() !== '') {
        ...
        this.loadArrayBuffer(`${this._modelHomeDir}${poseFileName}`, (arrayBuffer) => {
    ...
    const loadUserData = (): void => {
      if (this._modelSetting.getUserDataFile() !== '') {
        ...
        this.loadArrayBuffer(`${this._modelHomeDir}${userDataFile}`, (arrayBuffer) => {
  ...
  private setupTextures(): void {
    ...
    if (this._state === LoadStep.LoadTexture) {
      ...
      for (let modelTextureNumber = 0; modelTextureNumber < textureCount; modelTextureNumber++) {
        ...
        // ロード完了時に呼び出すコールバック関数
        const onLoad = (textureInfo: Live2DTexture): void => {
        ...
        this._subdelegate
          .getTextureManager()
          .createTextureFromPngFile(texturePath, usePremultiply, onLoad, (error) => this.failLoad(error));
  ...
  public update(): void {
    ...
    const deltaTimeSeconds: number = Live2DClock.getDeltaTime();
    ...
    // Eye openness belongs to the current motion/eye-blink frame. Persisting a
    // tap motion's final closed-eye keyframe can otherwise poison later idles.
    const eyeParameterBaseline = readParameterValues(this._model, this._eyeBlinkIds.get());
    if (this._motionManager.isFinished()) {
      // モーションの再生がない場合、待機モーションの中からランダムで再生する
      this.startRandomMotion(motionGroupIdle, motionPriority.idle);
    }
    // Apply a newly queued idle motion in the same frame so model-default
    // accessory visibility never leaks into the first rendered frame.
    motionUpdated = this._motionManager.updateMotion(this._model, deltaTimeSeconds); // モーションを更新
    saveParametersWithTransientState(this._model, eyeParameterBaseline); // 目以外の状態を保存
    //--------------------------------------------------------------------------
  ...
  public startMotion(
    ...
    if (motion == null) {
      CubismLogError(`Motion was not preloaded: ${name}`);
      this._motionManager.setReservePriority(motionPriority.none);
      return InvalidMotionQueueEntryHandleValue;
    }
    if (onBeganMotionHandler) motion.setBeganMotionHandler(onBeganMotionHandler);
  ...
  /** Returns every model-defined hit area containing the pointer. */
  public hitTestAll(x: number, y: number): string[] {
    if (this._opacity < 1) return [];
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (this.isHit(drawId, x, y)) hits.push(this._modelSetting.getHitAreaName(i));
    }

    return hits;
  }

  /** Whether the loaded model defines at least one motion in this group. */
  public hasMotionGroup(group: string): boolean {
    return this._modelSetting.getMotionCount(group) > 0;
  }
  ...
  private preLoadMotionGroup(group: string): void {
    for (let i = 0; i < this._modelSetting.getMotionCount(group); i++) {
      ...
      this.loadArrayBuffer(`${this._modelHomeDir}${motionFileName}`, (arrayBuffer) => {
        if (group === motionGroupIdle && i === 0) {
          this.applyInitialPartOpacities(arrayBuffer);
        }
  ...
  private applyInitialPartOpacities(motionBuffer: ArrayBuffer): void {
    try {
      const motion = JSON.parse(new TextDecoder().decode(motionBuffer)) as unknown;
      for (const { id, value } of readInitialPartOpacities(motion)) {
        this._model.setPartOpacityById(CubismFramework.getIdManager().getId(id), value);
      }
    } catch {
      CubismLogError('Failed to read initial part opacities from the idle motion.');
    }
  }
  ...
  private doDraw(): void {
    ...
    this.getRenderer().setRenderState(this._subdelegate.getFrameBuffer() as WebGLFramebuffer, viewport);
  ...
  public setRenderContext(subdelegate: Live2DRenderContext): void {
  ...
  public constructor() {
    ...
    this._motionCount = 0;
    this._allMotionCount = 0;
    this._wavFileHandler = new Live2DWavFileHandler();
  }

  private _subdelegate!: Live2DRenderContext;
  ...
  _wavFileHandler: Live2DWavFileHandler; //wavファイルハンドラ
}
